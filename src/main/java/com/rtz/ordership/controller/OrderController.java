...
public class OrderController {
    ...
    @Operation(summary = "Listar pedidos", description = "Lista pedidos paginados con filtros opcionales por estado y fecha de entrega")
    public ResponseEntity<Page<OrderResponse>> getAllOrders(
            @RequestParam(required = false) OrderStatus status,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate deliveryDate,
            @PageableDefault(size = 20, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {
        return ResponseEntity.ok(orderService.getAllOrders(status, deliveryDate, pageable));
    }
    ...
    @Operation(summary = "Crear pedido", description = "Crea un pedido con sus ítems, calcula totales automáticamente")
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'OPERATOR')")
    @Operation(summary = "Cancelar pedido", description = "Cancela un pedido (cambia el estado a CANCELLED)")
